...
function s.initial_effect(c)
	...
	c:RegisterEffect(e3)
		...
		-- EFEITO 05: Descartar 1 carta para destruir 1 monstro do oponente
...
function s.matfilter(c)
	return c:IsFaceup() and c:IsCode(107) and s.checkErenCondition(c)
end
...
function s.count_op(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:GetFlagEffect(id)==0 then
		...
		if ct==3 then
			...
			local eren=og:Filter(Card.IsCode,nil,107):GetFirst()
			if eren and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 then
				Duel.Overlay(c,og:Filter(aux.NOT(Card.IsCode),nil,107)) -- remove os outros materiais
...
function s.check_material(e,tp,eg,ep,ev,re,r,rp)
	...
	local og=c:GetOverlayGroup()
	if not og:IsExists(Card.IsCode,1,nil,107) then
		Duel.Destroy(c,REASON_EFFECT)
	end
end
...
function s.no_eren_cond(e,tp,eg,ep,ev,re,r,rp)
	...
	local og=c:GetOverlayGroup()
	return not og:IsExists(Card.IsCode,1,nil,107)
end
...
-- Custo: descartar 1 carta
function s.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD)
end

-- Seleciona monstro do oponente
